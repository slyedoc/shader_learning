...
use bevy::{math::vec2, prelude::*, render2::primitives::Aabb};
...
impl Plugin for ShapePlugin {
    fn build(&self, app: &mut App) {
        app
            .add_system(shape_change_detection_system)
            .add_system(mesh_detection_system::<Quad>)
            .add_system(mesh_detection_system::<Cube>)
            .add_system(mesh_detection_system::<Box>);
...
fn mesh_detection_system<T: Into<Mesh>>(
    mut commands: Commands,
    mut query: Query<(Entity, &T), Or<(Added<T>, Changed<T>)>>,
    mut meshes: ResMut<Assets<Mesh>>,
) where
    T: Send + Sync + Copy + Inspectable + 'static,
    Mesh: From<T>,
{
    for (e, shape) in query.iter_mut() {
        let m = Mesh::from(*shape);
        let h = meshes.add(m);
        commands.entity(e).remove::<Mesh>();
        commands.entity(e).insert(h);
    }
